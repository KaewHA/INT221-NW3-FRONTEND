...
import { getAnnouncementById,getcount } from '../assets/data.js'
    ...
    <div class="w-screen font-noto h-screen">
        ...
        <div v-if="announcement" class="w-full h-full  flex-row justify-center items-center">
            <div class="w-full flex justify-center bg-sky-600 pt-7 ">
                ...
                <div class="w-full flex justify-center"> <h1 class="text-left text-3xl mb-4 text-white">Announcement Detail</h1></div>
            </div>
            ...
            <div class="border-2  flex justify-center items-center w-3/4 h-3/4 rounded-2xl  shadow-2xl">
                <div class="ann-item w-full">
                    <div class="flex-row justify-start mt-3 pl-4 ">
                        <div class="flex text-white"><spam class="bg-emerald-400 flex rounded-3xl w-56 border"><pdate class="mt-1 ml-3"></pdate><span class="ann-publish-date ml-3"> {{ dateformat(announcement.publishDate) }} </span></spam></div>
                        
                        <div class="flex text-white"><spam class="bg-red-400 flex rounded-3xl w-56 border"><cdate class="mt-1 ml-3"></cdate><span class="ann-close-date ml-4"></span> {{  dateformat(announcement.closeDate) }} </spam></div>
                    </div>
                    <div class="text-2xl flex justify-end w-full pr-9 pt-5 -mt-16">
                        <span class="w-16 h-15 rounded-full ann-display text-center" :class="announcement.announcementDisplay == 'Y' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'">{{ announcement.announcementDisplay }}</span>
                    </div>
                ...
                <div class=" fixed top-1 left-0 flex justify-between p-4 ">
                ...
                <div class=" fixed flex bottom-0 w-full  justify-center p-4 ">
                    <views class="mt-1 mr-3"></views>VIEW : {{ count.count }}
                </div>
                <div class="fixed top-1 right-0 flex justify-between p-4">
