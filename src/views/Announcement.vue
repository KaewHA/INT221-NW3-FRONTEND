...
<template>
  <div class="w-screen h-screen">
    ...
    <div v-if="testdata.length!=0">
      <h1>Date/Time Showintimezone : {{ timezoneName }}</h1>
      <table class=" center w-full">
        <tr class="sticky top-0 border-2 pr-6">
          <th class=" ">No.</th>
        ...
        <tr v-for="(announcement, index) in allAnnouncement" :key="index" class="overflow-auto">
          ...
          <td class="text-center">
            <router-link :to="{ name: 'AnnouncementDetail', params: { id: announcement.announcementID }}">
              <button class="rounded-md bg-gray-200 px-3">view</button>
            </router-link>
          </td>
        </tr>
      </table>
    ...
    <div v-else>
        <h1 class="text-4xl flex w-full items-center justify-center ">No Announcement</h1>
    </div>
  </div>
</template>
